...
{
    public class ScoreAmountPhone : ParameterBase
    {
        public ScoreAmountPhone(Areas.Chat.Models.UserSession userSession, string sentence)
            : base(userSession, sentence)
        {
            ...
            ValueRegexs = new string[] {
                @"(?<=手机.*)(好)",
                @"好(?=.*手机)",
                @"(?<=手机)如何|(?<=手机)怎么样|(?<=手机)怎样|(?<=手机)好不|(?<=手机)行不",
                @"随意|随便|都可以|其他|别的"
            };
            #endregion
        ...
        public override void Format()
        {
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                {
                    if (userSession.liveTable.Parameter["手机总得分"] != null)
                    {
                        String[] score = ((String)userSession.liveTable.Parameter["手机总得分"]).Split(',');
