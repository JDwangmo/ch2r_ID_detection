...
{ 
    public class WLan : ParameterBase
    {
        public WLan (Areas.Chat.Models.UserSession userSession, string sentence) : base(userSession, sentence)
        {
            name = "WLAN";
        ...
        public override void Format()
        {
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                {
                    if (userSession.liveTable.Parameter["WLAN"] != null)
                    {
                        list[i].normalData = (String)userSession.liveTable.Parameter["WLAN"];
