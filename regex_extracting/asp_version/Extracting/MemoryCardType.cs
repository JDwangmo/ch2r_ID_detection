...
{ 
    public class MemoryCardType : ParameterBase
    {
        public MemoryCardType (Areas.Chat.Models.UserSession userSession, string sentence)
        ...
        public override void Format()
        {
            for (int i = 0; i < list.Count; ++i)
            {
                ...
                {
                    if (userSession.liveTable.Parameter["存储卡类型"] != null)
                    {
                        list[i].normalData = (String)userSession.liveTable.Parameter["存储卡类型"];
